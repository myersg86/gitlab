  ...
  describe 'callbacks' do
    ...
    context 'truncate_fields' do
      shared_examples 'a truncated field' do
        ...
        with_them do
          let(:text_limit) { described_class::TEXT_LIMIT[field_name] }
          let(:values) do
            {
              long: 'a' * (text_limit + 1),
              short: 'a' * text_limit,
              truncated: 'a' * (text_limit - 3) + '...'
            }
          end

          let(:audit_event) do
            create(:audit_event, field_name => values[database_column], details: { field_name => values[details_value] })
          end
