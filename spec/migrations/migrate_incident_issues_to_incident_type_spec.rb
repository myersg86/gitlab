...
RSpec.describe MigrateIncidentIssuesToIncidentType do
  ...
  let(:issue_issue_type) { 0 }
  let(:incident_issue_type) { 1 }
  ...
  describe '#up' do
    it 'updates the incident issue type' do
      expect { migrate! }
        ...
        .from(issue_issue_type)
        .to(incident_issue_type)

      expect(other_issue.reload.issue_type).to eql(issue_issue_type)
    end
  end
