...
class RemoveDuplicateLabelsFromGroup < ActiveRecord::Migration[6.0]
  ...
  def up
    ...
    Group.each_batch(of: BATCH_SIZE) do |batch|
...
WITH backups AS (
  SELECT id, title
  FROM backup_labels
  WHERE group_id BETWEEN #{start_id} AND #{stop_id} AND
...
INSERT INTO labels
SELECT id, title, color, group_id, created_at, updated_at, template, description, description_html, type, group_id, cached_markdown_version FROM backup_labels
  WHERE backup_labels.group_id BETWEEN #{start_id} AND #{stop_id}
